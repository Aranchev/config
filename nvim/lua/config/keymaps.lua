...
vim.keymap.set(
  "n",
  "<leader>pt",
  ':w<CR>:split | terminal bash -c "export PYTHONPATH=/mnt/c/Users/Emi\\\\ Nikolova/pytest_fundamental && pytest \\"%\\""<CR>',
...
vim.keymap.set("n", "<leader>d1", function()
  vim.cmd.cd("/mnt/c/Users/Emi Nikolova/My Drive/My folder/Obsidian")
...
vim.keymap.set("n", "<leader>d3", function()
  vim.cmd.cd("/mnt/c/Users/Emi Nikolova/microblog")
end, { desc = "set dir: to microblog" })

vim.keymap.set("n", "<leader>d4", function()
  vim.cmd.cd(vim.fn.expand("~/.config"))
end, { desc = "set dir: .config" })

-- Text formatting (no changes)
...
-- Comment and Uncomment commands (unchanged)
